/**
 * Mirrors client tax logic for server-side payment accruals. Illustrative only.
 */
...
function federalIncomeTaxBrackets(taxable, brackets) {
...
function estimateSelfEmploymentTax(netSelfEmploymentIncome) {
...
function estimateTotalTaxes({
  ...
  const total = se.total + federalIncome + stateIncome;

  const filingNote =
    entityType === 's_corp'
      ? 'S-corp (simplified: no SE tax on pass-through; salary not modeled)'
...
module.exports = { estimateTotalTaxes, estimateSelfEmploymentTax };
